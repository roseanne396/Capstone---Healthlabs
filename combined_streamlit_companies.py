...
# Multi HyDE helper functions
def build_hyde_queries(api_key, model_name, strategy_desc, hypothetical_doc, clean_target_doc):
    """
    Build multiple synthetic queries (multi-HyDE) for a given pillar.
    Returns a list of query strings:
      1) detailed product-style HyDE (already generated by profiler_chain)
      2) short keyword-style query derived from that HyDE + strategy
    """
    # 1) keep the long, detailed HyDE as-is
    hyde_detailed = hypothetical_doc

    # 2) generate a short, keyword-style query
    ...
    You are helping construct a dense search query for an embedding-based
    retrieval system over digital health products.

    Below is a synthetic product description (HyDE) and the partnership pillar
    it was generated from.

    --- Synthetic HyDE Product Description ---
    {hyde_doc}

    --- Partnership Strategy / Pillar Description ---
    {strategy_desc}

    Task:
    - Output a SINGLE LINE of comma-separated key phrases.
    - No full sentences.
    - Focus on: clinical area, user personas, care setting, core features,
      data types, tech stack, and important synonyms.
    - Do NOT number the items.

    Example output format:
    diabetes remote monitoring, CGM integration, endocrinology clinics, 
    EHR integration, population health dashboard
    """)

    keyword_chain = keyword_prompt | keyword_llm | StrOutputParser()

    try:
        hyde_keywords = keyword_chain.invoke({
            "hyde_doc": hyde_detailed,
            "strategy_desc": strategy_desc,
        })
    except Exception as e:
        # Fallback: if the extra call fails, just reuse the long HyDE
        hyde_keywords = hyde_detailed

    return [hyde_detailed, hyde_keywords]


def multi_hyde_retriever(_vectorstore, hyde_queries, k_each: int = 12):
    """
    Run similarity search for EACH HyDE query and merge the results.
    De-duplicates by (Company, Product, partial content).
    """
    all_docs = []
    for q in hyde_queries:
        if not q:
            continue
        docs = _vectorstore.similarity_search(q, k=k_each)
        all_docs.extend(docs)

    # de-duplicate across all HyDE variants
    unique = {}
    for doc in all_docs:
        company = doc.metadata.get("Company")
        product = doc.metadata.get("Product")
        snippet = doc.page_content[:80]  # first 80 chars as cheap chunk id
        key = (company, product, snippet)
...
def run_pipeline_execution_p1(api_key, merged_data, company_to_products_map, target_product_input, num_angles, _vectorstore, model_name):
    ...
    # --- NEW: build multi-HyDE queries (detailed + keyword-style) ---
        hyde_queries = build_hyde_queries(
            api_key=api_key,
            model_name=model_name,
            strategy_desc=strategy_desc,
            hypothetical_doc=hypothetical_doc,
            clean_target_doc=clean_target_doc,
        )

        # --- NEW: multi-HyDE retrieval over the vector store ---
        # k_each controls how many chunks per HyDE variant (2 variants × 12 = up to 24 chunks/pillar)
        retrieved_docs = multi_hyde_retriever(
